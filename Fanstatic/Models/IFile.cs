...
public interface IFile
{
    ...
    /// </summary>
    string SourceRelativePathDirectory => UrlExtension.NormalizeToUnix(Path.GetDirectoryName(SourceRelativePath));
    ...
    /// The source filename, without the extension. ;)
    /// </summary>
    string SourceFullPath(string basePath) => Path.Combine(basePath, SourceRelativePath);

    /// <summary>
    /// The full source directory of the file, without the file name.
    /// </summary>
    string SourceFullPathDirectory(string basePath) =>
        UrlExtension.NormalizeToUnix(Path.GetDirectoryName(SourceFullPath(basePath)));
}
